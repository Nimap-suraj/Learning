...
//            while (reader.Read())
//            {
//                ids.Add(Convert.ToInt32(reader[pkColumn]));
//            }
//            return ids;
//        }
...
//                {
//                    row[reader.GetName(i)] = reader.IsDBNull(i) ? null : 
//                        reader.GetValue(i);
//                }
...
//        //        bool dataIsDifferent = (!row["Name"].Equals(targetRecord["Name"])) ||
//        //                           (!row["Email"].Equals(targetRecord["Email"])); ;
...
//            if (isServerToClient)
//            {
//                // Null-safe comparison of both fields
//                bool nameChanged = !Equals(row["Name"]?.ToString() ?? "", targetRecord["Name"]?.ToString() ?? "");
//                bool emailChanged = !Equals(row["Email"]?.ToString() ?? "", targetRecord["Email"]?.ToString() ?? "");
//                bool dataIsDifferent = nameChanged || emailChanged;
